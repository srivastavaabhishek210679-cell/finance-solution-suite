    ...
    <div style={{minHeight:'100vh',background:'#0f172a',color:'#f1f5f9',fontFamily:'Inter,sans-serif'}}>
      ...
      <div style={{padding:24}}>
          ...
          <div style={{background:'#ef444415',border:'1px solid #ef444440',borderRadius:12,padding:16,marginBottom:20}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8}}>
              {stats.expiringSoon.map(c=>{
                ...
                return (
                  <div key={c.contract_id} style={{background:'#0f172a',borderRadius:8,padding:10}}>
                    <div style={{color:'#f1f5f9',fontWeight:600,fontSize:12}}>{c.contract_name}</div>
        ...
        <div style={{display:'flex',gap:12,marginBottom:16}}>
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search contracts..." style={{flex:1,background:'#1e293b',border:'1px solid #334155',borderRadius:8,color:'#f1f5f9',padding:'10px 14px',fontSize:13}}/>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:16}}>
          {filtered.map(c=>{
            const days = getDaysUntilExpiry(c.end_date)
            return (
              <div key={c.contract_id} style={{background:'#1e293b',border:`1px solid ${days<30&&c.status==='Active'?'#ef444460':'#334155'}`,borderRadius:12,padding:20}}>
                <div style={{display:'flex',justifyContent:'space-between',marginBottom:12}}>
                  <div>
                    <div style={{color:'#f1f5f9',fontWeight:700,fontSize:14}}>{c.contract_name}</div>
                ...
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8,marginBottom:12,fontSize:12}}>
                  <div><span style={{color:'#64748b'}}>Type: </span><span style={{color:'#94a3b8'}}>{c.contract_type}</span></div>
                  <div><span style={{color:'#64748b'}}>Dept: </span><span style={{color:'#94a3b8'}}>{c.department}</span></div>
                  ...
                  <div><span style={{color:'#64748b'}}>End: </span><span style={{color:days<30&&c.status==='Active'?'#ef4444':'#94a3b8'}}>{c.end_date?.slice(0,10)}</span></div>
                  <div><span style={{color:'#64748b'}}>Signed: </span><span style={{color:'#94a3b8'}}>{c.signed_by}</span></div>
                </div>
                ...
                <button onClick={()=>{setEditContract(c);setForm({...c,vendor_id:c.vendor_id||''});setShowForm(true)}} style={{background:'#334155',border:'none',borderRadius:6,color:'#94a3b8',padding:'4px 10px',cursor:'pointer',fontSize:12,display:'flex',alignItems:'center',gap:4}}><Edit size={12}/> Edit</button>
              </div>
            )
          })}
        </div>
      ...
      {showForm && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={()=>setShowForm(false)}>
          <div style={{background:'#1e293b',border:'1px solid #334155',borderRadius:16,padding:24,width:580,maxHeight:'90vh',overflowY:'auto'}} onClick={e=>e.stopPropagation()}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}><h2 style={{color:'#f1f5f9',margin:0}}>{editContract?'Edit Contract':'New Contract'}</h2><button onClick={()=>setShowForm(false)} style={{background:'none',border:'none',color:'#94a3b8',cursor:'pointer'}}><X size={20}/></button></div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
              ...
              <div><label style={{fontSize:11,color:'#64748b',display:'block',marginBottom:4}}>Vendor</label><select value={form.vendor_id} onChange={e=>setForm({...form,vendor_id:e.target.value})} style={{width:'100%',background:'#0f172a',border:'1px solid #334155',borderRadius:8,color:'#f1f5f9',padding:'8px 12px',fontSize:13}}><option value="">No Vendor</option>{vendors.map(v=><option key={v.vendor_id} value={v.vendor_id}>{v.vendor_name}</option>)}</select></div>
              <div><label style={{fontSize:11,color:'#64748b',display:'block',marginBottom:4}}>Contract Type</label><select value={form.contract_type} onChange={e=>setForm({...form,contract_type:e.target.value})} style={{width:'100%',background:'#0f172a',border:'1px solid #334155',borderRadius:8,color:'#f1f5f9',padding:'8px 12px',fontSize:13}}>{CONTRACT_TYPES.map(t=><option key={t}>{t}</option>)}</select></div>
              ...
              <div><label style={{fontSize:11,color:'#64748b',display:'block',marginBottom:4}}>Status</label><select value={form.status} onChange={e=>setForm({...form,status:e.target.value})} style={{width:'100%',background:'#0f172a',border:'1px solid #334155',borderRadius:8,color:'#f1f5f9',padding:'8px 12px',fontSize:13}}>{['Active','Draft','Expired','Terminated'].map(s=><option key={s}>{s}</option>)}</select></div>
              <div style={{display:'flex',alignItems:'center',gap:8,paddingTop:20}}><input type="checkbox" checked={form.auto_renewal} onChange={e=>setForm({...form,auto_renewal:e.target.checked})} style={{width:16,height:16}}/><label style={{color:'#94a3b8',fontSize:13}}>Auto Renewal</label></div>
