    ...
    <div style={{minHeight:'100vh',background:'#f8fafc',color:'#0f172a',fontFamily:'Inter,sans-serif'}}>
      ...
      <div style={{padding:24}}>
        ...
        <div style={{display:'flex',gap:12,marginBottom:16}}>
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search travel requests..." style={{flex:1,background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:8,color:'#0f172a',padding:'10px 14px',fontSize:13}}/>
        ...
        <div style={{display:'grid',gap:12}}>
          {filtered.map(r=>(
            <div key={r.travel_id} style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:12,padding:20}}>
              <div style={{display:'flex',justifyContent:'space-between',marginBottom:12}}>
                <div style={{display:'flex',gap:12,alignItems:'center'}}>
                  ...
                  <div>
                    <div style={{color:'#0f172a',fontWeight:700,fontSize:14}}>{r.employee_name} → {r.destination}</div>
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:8,fontSize:12}}>
                <div><span style={{color:'#64748b'}}>Departure: </span><span style={{color:'#475569'}}>{r.departure_date?.slice(0,10)}</span></div>
                <div><span style={{color:'#64748b'}}>Return: </span><span style={{color:'#475569'}}>{r.return_date?.slice(0,10)}</span></div>
                ...
                <div><span style={{color:'#64748b'}}>Hotel: </span><span style={{color:'#475569'}}>{r.hotel_required?'Required':'Not Required'}</span></div>
                <div><span style={{color:'#64748b'}}>Advance: </span><span style={{color:'#f59e0b'}}>₹{Number(r.advance_required).toLocaleString()}</span></div>
              </div>
              {r.purpose && <div style={{background:'#f8fafc',borderRadius:8,padding:10,marginTop:10,fontSize:12,color:'#475569'}}><span style={{color:'#64748b'}}>Purpose: </span>{r.purpose}</div>}
      ...
      {showForm && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={()=>setShowForm(false)}>
          <div style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:16,padding:24,width:580,maxHeight:'90vh',overflowY:'auto'}} onClick={e=>e.stopPropagation()}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}><h2 style={{color:'#0f172a',margin:0}}>New Travel Request</h2><button onClick={()=>setShowForm(false)} style={{background:'none',border:'none',color:'#475569',cursor:'pointer'}}><X size={20}/></button></div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
              ...
              <div><label style={{fontSize:11,color:'#64748b',display:'block',marginBottom:4}}>Travel Mode</label><select value={form.travel_mode} onChange={e=>setForm({...form,travel_mode:e.target.value})} style={{width:'100%',background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:8,color:'#0f172a',padding:'8px 12px',fontSize:13}}>{TRAVEL_MODES.map(m=><option key={m}>{m}</option>)}</select></div>
              <div style={{display:'flex',alignItems:'center',gap:8,paddingTop:20}}><input type="checkbox" checked={form.hotel_required} onChange={e=>setForm({...form,hotel_required:e.target.checked})} style={{width:16,height:16}}/><label style={{color:'#475569',fontSize:13}}>Hotel Required</label></div>
              <div style={{gridColumn:'span 2'}}><label style={{fontSize:11,color:'#64748b',display:'block',marginBottom:4}}>Purpose</label><textarea value={form.purpose||''} onChange={e=>setForm({...form,purpose:e.target.value})} rows={2} style={{width:'100%',background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:8,color:'#0f172a',padding:'8px 12px',fontSize:13,boxSizing:'border-box',resize:'vertical'}}/></div>
            </div>
