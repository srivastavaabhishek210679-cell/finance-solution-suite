      ...
      {isConn && (
        <>
          ...
          <div className="ie-conn-stats">
            <div className="ie-conn-stat">
              <Database size={11}/>
              <span>{conn.records.toLocaleString()} records</span>
            </div>
            <div className="ie-conn-stat">
              <Clock size={11}/>
          ...
          <div className="ie-conn-last-sync">
            <RefreshCw size={10}/> Last sync: {conn.lastSync}
          </div>
        </>
      )}
...
export default function IntegrationEcosystem() {
  ...
  const handleConnect    = (id) => { setConnectors(p => p.map(c => c.id===id ? {...c, status:'connected', health:88, records:0, lastSync:'Just now'} : c)); showToast('Integration connected successfully!') }
  const handleDisconnect = (id) => { setConnectors(p => p.map(c => c.id===id ? {...c, status:'disconnected', health:0, records:0, lastSync:'Never'} : c)); showToast('Integration disconnected', 'warning') }
  const handleConfigure  = (id) => showToast('Configuration panel — coming in Phase 8')
  ...
  const filteredLogs = SYNC_LOGS.filter(l => {
    const stOk  = logFilter.status   === 'All' || l.status   === logFilter.status
    const catOk = logFilter.category === 'All' || l.category === logFilter.category
    return stOk && catOk
  })
  ...
  const logStatusColor = { success:'#10b981', failed:'#ef4444', warning:'#f59e0b' }
              ...
              {connectors.map(c => {
                ...
                return (
                  <div key={c.id} className="ie-health-row-item">
                    ...
                    <span className="ie-health-status-badge" style={{ background:cfg.bg, color:cfg.color }}>
            ...
            <div className="ie-mini-logs">
              {SYNC_LOGS.slice(0,6).map(l => (
                <div key={l.id} className="ie-mini-log-row">
                  <span className="ie-mini-log-status" style={{ color:logStatusColor[l.status] }}>
          ...
          {showWHForm && (
            <div className="ie-wh-form">
              ...
              <div className="ie-wh-form-grid">
                <div className="ie-form-group full">
                  <label className="ie-label">Webhook Name</label>
                  <input className="ie-input" placeholder="e.g. Report Created Notification" value={whForm.name} onChange={e=>setWHForm(p=>({...p,name:e.target.value}))}/>
                </div>
                <div className="ie-form-group full">
                  <label className="ie-label">Endpoint URL</label>
                  <input className="ie-input" placeholder="https://api.example.com/webhooks/..." value={whForm.url} onChange={e=>setWHForm(p=>({...p,url:e.target.value}))}/>
                </div>
                <div className="ie-form-group">
                  <label className="ie-label">Direction</label>
                  <select className="ie-select" value={whForm.direction} onChange={e=>setWHForm(p=>({...p,direction:e.target.value}))}>
                    <option value="outbound">Outbound (we send)</option>
                    <option value="inbound">Inbound (we receive)</option>
                  </select>
                </div>
                <div className="ie-form-group">
                  <label className="ie-label">Event Type</label>
            ...
            {webhooks.map(wh => (
              <div key={wh.id} className="ie-wh-card">
                ...
                <div className="ie-wh-secret">
                  ...
                  <button className="ie-wh-secret-btn" onClick={()=>setShowSecrets(p=>({...p,[wh.id]:!p[wh.id]}))}>
                    {showSecrets[wh.id] ? <EyeOff size={11}/> : <Eye size={11}/>}
                  </button>
                  <button className="ie-wh-secret-btn" onClick={()=>{ navigator.clipboard.writeText(wh.secret); showToast('Secret copied!') }}>
                    <Copy size={11}/>
                  </button>
                </div>
              </div>
        ...
        <div className="ie-content">
          <div className="ie-connectors-header">
            ...
            <div className="ie-log-filters">
              {[
                { key:'status',   opts:['All','success','failed','warning'], label:'Status'   },
                { key:'category', opts:['All','ERP','CRM','HRMS'],           label:'Category' },
              ].map(f => (
          ...
          <div className="ie-logs-table-wrap">
            <div className="ie-logs-head">
              <span>System</span><span>Category</span><span>Type</span>
              <span>Status</span><span>Records</span><span>Duration</span>
              <span>Size</span><span>Timestamp</span>
            </div>
            {filteredLogs.map(l => (
              <div key={l.id} className="ie-log-row">
                ...
                <span className="ie-log-status" style={{ color:logStatusColor[l.status], background:`${logStatusColor[l.status]}18` }}>
                  {l.status==='success'?<CheckCircle size={11}/>:l.status==='failed'?<XCircle size={11}/>:<AlertTriangle size={11}/>}
                  {l.status}
                </span>
          ...
          <div className="ie-log-stats">
            <div className="ie-log-stat-card" style={{ borderColor:'#10b981' }}>
              <div style={{ fontSize:24, fontWeight:800, color:'#10b981' }}>{SYNC_LOGS.filter(l=>l.status==='success').length}</div>
              <div style={{ fontSize:11, color:'#64748b' }}>Successful Syncs</div>
            </div>
            <div className="ie-log-stat-card" style={{ borderColor:'#ef4444' }}>
              <div style={{ fontSize:24, fontWeight:800, color:'#ef4444' }}>{SYNC_LOGS.filter(l=>l.status==='failed').length}</div>
              <div style={{ fontSize:11, color:'#64748b' }}>Failed Syncs</div>
            </div>
            <div className="ie-log-stat-card" style={{ borderColor:'#f59e0b' }}>
              <div style={{ fontSize:24, fontWeight:800, color:'#f59e0b' }}>{SYNC_LOGS.filter(l=>l.status==='warning').length}</div>
              <div style={{ fontSize:11, color:'#64748b' }}>Warnings</div>
            </div>
            <div className="ie-log-stat-card" style={{ borderColor:'#3b82f6' }}>
              <div style={{ fontSize:24, fontWeight:800, color:'#3b82f6' }}>{SYNC_LOGS.reduce((s,l)=>s+l.records,0).toLocaleString()}</div>
              <div style={{ fontSize:11, color:'#64748b' }}>Total Records Synced</div>
            </div>
          </div>
