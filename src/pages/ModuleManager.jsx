...
import { Package, Search, Download, Trash2, Settings, CheckCircle, XCircle, AlertCircle, RefreshCw, ArrowLeft, BarChart3 } from 'lucide-react'
    ...
    <div className="module-manager-page">
      <div className="module-header">
        ...
        <div className="header-content">
          ...
          <button className="btn-primary" onClick={() => setShowMarketplace(true)}>
            <Download size={18} /> Browse Marketplace
          </button>
        </div>
      </div>

      <div className="stats-grid">
        <div className="stat-card"><div className="stat-icon" style={{background:'#3b82f620',color:'#3b82f6'}}><Package size={20}/></div><div className="stat-content"><div className="stat-value">{modules.length}</div><div className="stat-label">Installed Modules</div></div></div>
        <div className="stat-card"><div className="stat-icon" style={{background:'#10b98120',color:'#10b981'}}><CheckCircle size={20}/></div><div className="stat-content"><div className="stat-value">{modules.filter(m=>m.status==='active').length}</div><div className="stat-label">Active Modules</div></div></div>
        <div className="stat-card"><div className="stat-icon" style={{background:'#ef444420',color:'#ef4444'}}><XCircle size={20}/></div><div className="stat-content"><div className="stat-value">{modules.filter(m=>m.status==='inactive').length}</div><div className="stat-label">Inactive Modules</div></div></div>
        <div className="stat-card"><div className="stat-icon" style={{background:'#8b5cf620',color:'#8b5cf6'}}><BarChart3 size={20}/></div><div className="stat-content"><div className="stat-value">{modules.reduce((s,m)=>s+m.reports,0)}</div><div className="stat-label">Total Reports</div></div></div>
      </div>
      ...
      <div className="filters-section">
        ...
        <div className="filter-tabs">
          {[['all','All Categories'],['report','Report Modules'],['compliance','Compliance'],['dashboard','Dashboard'],['integration','Integrations']].map(([val,label]) => (
            <button key={val} className={filterCategory===val?'active':''} onClick={() => setFilterCategory(val)}>{label}</button>
          ))}
        </div>
        <div className="status-filters">
          {['all','active','inactive'].map(s => (
            <button key={s} className={filterStatus===s?'active':''} onClick={() => setFilterStatus(s)}>{s.charAt(0).toUpperCase()+s.slice(1)}</button>
          ))}
        </div>
      </div>
            ...
            <div key={mod.id} className={'module-card ' + mod.status}>
              <div className="module-header">
                <div className="module-info">
                  <Package size={20} className="module-icon"/>
                  <h3>{mod.name}</h3>
                  <span className="version-badge">v{mod.version}</span>
                </div>
                <div className={'status-indicator ' + mod.status}>
                  {mod.status==='active' ? <CheckCircle size={16}/> : <XCircle size={16}/>}
                </div>
              </div>
              <p className="module-description">{mod.description}</p>
              <div className="module-meta">
                <span className="meta-item">{mod.reports} reports</span>
              ...
              <div className="module-actions">
                ...
                <button onClick={() => alert('Opening configuration...')} className="config-btn"><Settings size={14}/></button>
                <button onClick={() => alert('Downloading latest version...')} className="update-btn"><RefreshCw size={14}/> Update</button>
                <button onClick={() => uninstallModule(mod.id)} className="uninstall-btn"><Trash2 size={14}/> Uninstall</button>
              </div>
            </div>
          ))}
        </div>
          ...
          {availableModules.map(mod => (
            <div key={mod.id} className="module-card available">
              <div className="module-header">
                ...
                <span className="price-badge">{mod.price}</span>
              </div>
              <p className="module-description">{mod.description}</p>
              <div className="module-meta">
                <span className="meta-item">{mod.reports} reports</span>
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.7)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center'}} onClick={() => setShowMarketplace(false)}>
          <div style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:12,padding:24,width:600,maxHeight:'80vh',overflowY:'auto'}} onClick={e => e.stopPropagation()}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
              <h2 style={{color:'#1e293b',margin:0}}>Module Marketplace</h2>
              ...
              {MARKETPLACE.map((m,i) => (
                <div key={i} style={{background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:10,padding:16}}>
                  <div style={{display:'flex',justifyContent:'space-between',marginBottom:8}}>
                    <span style={{color:'#1e293b',fontWeight:600,fontSize:13}}>{m.name}</span>
