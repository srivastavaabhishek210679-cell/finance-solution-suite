...
function ModuleManager() {
  ...
  const categories = [
    ...
    { id: 'report', name: 'Report Modules', icon: '=ƒôè' },
    { id: 'chart', name: 'Chart Modules', icon: '=ƒôê' },
    { id: 'integration', name: 'Integrations', icon: '=ƒöù' },
    { id: 'custom', name: 'Custom Modules', icon: 'GÜÖn+Å' }
  ]
            ...
            <div key={module.id} className={`module-card ${module.status}`}>
              ...
              <div className="module-meta">
                <span className="meta-item">
                  {module.category === 'report' && '=ƒôè'}
                  {module.category === 'chart' && '=ƒôê'}
                  {module.category === 'integration' && '=ƒöù'}
                  {module.category === 'custom' && 'GÜÖn+Å'}
                  {module.category}
                </span>
                {module.reports > 0 && (
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.7)", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center" }} onClick={() => setShowMarketplace(false)}>
          <div style={{ background:"#ffffff", border:"1px solid #e2e8f0", borderRadius:12, padding:24, width:600, maxHeight:"80vh", overflowY:"auto" }} onClick={e => e.stopPropagation()}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
              ...
              <button onClick={() => setShowMarketplace(false)} style={{ background:"none", border:"none", color:"#475569", cursor:"pointer", fontSize:20 }}>+</button>
            </div>
