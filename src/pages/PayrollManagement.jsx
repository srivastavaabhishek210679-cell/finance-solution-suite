...
export default function PayrollManagement() {
  ...
  const s = { container:{ minHeight:'100vh', background:'#f8fafc', color:'#0f172a', fontFamily:'Inter,sans-serif' }, header:{ background:'#ffffff', borderBottom:'1px solid #e2e8f0', padding:'16px 24px', display:'flex', alignItems:'center', gap:16 }, content:{ padding:24 }, card:{ background:'#ffffff', border:'1px solid #e2e8f0', borderRadius:12, padding:20 } }
      ...
      <div style={s.content}>
        ...
        {stats?.departmentCosts?.length > 0 && (
          <div style={{...s.card, marginBottom:24}}>
            <h3 style={{color:'#0f172a',marginBottom:16,fontSize:14}}>Department Payroll Cost</h3>
        ...
        {tab === 'employees' && (
          <div style={s.card}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
              <h3 style={{color:'#0f172a',margin:0}}>Employees ({filteredEmployees.length})</h3>
              <div style={{display:'flex',alignItems:'center',gap:8,background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:8,padding:'8px 12px'}}>
                ...
                <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search employees..." style={{background:'none',border:'none',color:'#0f172a',fontSize:13,outline:'none',width:200}}/>
              </div>
            </div>
                ...
                {filteredEmployees.map(emp => {
                  ...
                  return (
                    <tr key={emp.employee_id} style={{borderBottom:'1px solid #1e293b'}}>
                      ...
                      <td style={{padding:'12px 8px',color:'#0f172a',fontWeight:600,fontSize:13}}>{emp.first_name} {emp.last_name}</td>
        ...
        {tab === 'payroll' && (
          <div style={s.card}>
            <h3 style={{color:'#0f172a',margin:'0 0 16px'}}>Payroll Runs</h3>
                ...
                <tbody>
                  {payrollRuns.map(run => (
                    <tr key={run.payroll_id} style={{borderBottom:'1px solid #1e293b'}}>
                      <td style={{padding:'12px 8px',color:'#0f172a',fontWeight:600}}>{MONTHS[run.month-1]} {run.year}</td>
            ...
            {!reportsData ? (
              ...
              <div>
                <div style={{background:"#ffffff",border:"1px solid #e2e8f0",borderRadius:12,padding:20,marginBottom:20}}>
                  <h3 style={{color:"#0f172a",marginBottom:16,fontSize:14}}>Payroll Cost by Department</h3>
                  <table style={{width:"100%",borderCollapse:"collapse"}}>
                ...
                <div style={{background:"#ffffff",border:"1px solid #e2e8f0",borderRadius:12,padding:20,marginBottom:20}}>
                  ...
                  <table style={{width:"100%",borderCollapse:"collapse"}}>
                    ...
                    <tbody>{reportsData.salaryBands.map((s,i)=>(<tr key={i} style={{borderBottom:"1px solid #f1f5f9"}}><td style={{padding:"10px 8px",color:"#0f172a",fontWeight:600}}>{s.designation}</td><td style={{padding:"10px 8px",color:"#475569"}}>{s.count}</td><td style={{padding:"10px 8px",color:"#10b981",fontWeight:600}}>{"?"+Number(s.avg_salary).toLocaleString()}</td></tr>))}</tbody>
                  </table>
                </div>
                {reportsData.trend.length > 0 && (
                  <div style={{background:"#ffffff",border:"1px solid #e2e8f0",borderRadius:12,padding:20}}>
                    <h3 style={{color:"#0f172a",marginBottom:16,fontSize:14}}>Monthly Payroll Trend</h3>
                    <table style={{width:"100%",borderCollapse:"collapse"}}>
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={()=>setShowForm(false)}>
          <div style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:16,padding:24,width:600,maxHeight:'90vh',overflowY:'auto'}} onClick={e=>e.stopPropagation()}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
              <h2 style={{color:'#0f172a',margin:0}}>{editEmp?'Edit Employee':'Add Employee'}</h2>
              ...
              {[
                ...
                <div key={key}>
                  ...
                  <input type={type} value={form[key]||''} onChange={e=>setForm({...form,[key]:e.target.value})}
                    style={{width:'100%',background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:8,color:'#0f172a',padding:'8px 12px',fontSize:13,boxSizing:'border-box'}}/>
                </div>
              ))}
              <div>
                ...
                <select value={form.department||'Finance'} onChange={e=>setForm({...form,department:e.target.value})}
                  style={{width:'100%',background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:8,color:'#0f172a',padding:'8px 12px',fontSize:13}}>
      ...
      {showPayroll && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center'}} onClick={()=>setShowPayroll(false)}>
          <div style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:16,padding:24,width:400}} onClick={e=>e.stopPropagation()}>
            <h2 style={{color:'#0f172a',marginBottom:20}}>Run Payroll</h2>
            ...
            <div style={{background:'#f8fafc',borderRadius:8,padding:12,marginBottom:20,fontSize:13,color:'#475569'}}>
              This will process payroll for <strong style={{color:'#0f172a'}}>{stats?.totalEmployees||0} active employees</strong> for <strong style={{color:'#10b981'}}>{MONTHS[payrollMonth-1]} {payrollYear}</strong>
            </div>
      ...
      {selectedRun && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={()=>setSelectedRun(null)}>
          <div style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:16,padding:24,width:800,maxHeight:'90vh',overflowY:'auto'}} onClick={e=>e.stopPropagation()}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
              <h2 style={{color:'#0f172a',margin:0}}>Payslips — {MONTHS[selectedRun.month-1]} {selectedRun.year}</h2>
            ...
            <table style={{width:'100%',borderCollapse:'collapse'}}>
                ...
                {payslips.map(p=>(
                  <tr key={p.payslip_id} style={{borderBottom:'1px solid #f1f5f9'}}>
                    <td style={{padding:'10px 8px'}}>
                      <div style={{color:'#0f172a',fontWeight:600,fontSize:13}}>{p.first_name} {p.last_name}</div>
              ...
              <tfoot>
                <tr style={{borderTop:'2px solid #334155',background:'#f8fafc'}}>
                  <td colSpan={4} style={{padding:'12px 8px',color:'#0f172a',fontWeight:700}}>TOTAL</td>
