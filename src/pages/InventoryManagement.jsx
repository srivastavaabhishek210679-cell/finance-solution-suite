    ...
    <div style={{minHeight:'100vh',background:'#0f172a',color:'#f1f5f9',fontFamily:'Inter,sans-serif'}}>
      ...
      <div style={{padding:24}}>
          ...
          <div style={{background:'#ef444415',border:'1px solid #ef444440',borderRadius:12,padding:16,marginBottom:20}}>
            ...
            <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
              {items.filter(i=>i.current_stock<=i.minimum_stock).map(i=>(
                <span key={i.item_id} style={{background:'#0f172a',color:'#f59e0b',padding:'4px 10px',borderRadius:20,fontSize:12}}>{i.item_name}: {i.current_stock} {i.unit} remaining</span>
              ))}
            </div>
          </div>
        )}
        ...
        <div style={{display:'flex',gap:12,marginBottom:16,alignItems:'center',flexWrap:'wrap'}}>
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search inventory..." style={{flex:1,minWidth:200,background:'#1e293b',border:'1px solid #334155',borderRadius:8,color:'#f1f5f9',padding:'10px 14px',fontSize:13}}/>
        ...
        <div style={{background:'#1e293b',border:'1px solid #334155',borderRadius:12,padding:20}}>
          <h3 style={{color:'#f1f5f9',margin:'0 0 16px',fontSize:14}}>Inventory Items ({filtered.length})</h3>
          <table style={{width:'100%',borderCollapse:'collapse'}}>
            <thead><tr style={{borderBottom:'1px solid #334155'}}>{['Item','Code','Category','Stock','Min/Max','Unit Price','Value','Supplier','Status','Actions'].map(h=><th key={h} style={{color:'#64748b',fontSize:11,padding:'8px',textAlign:'left',textTransform:'uppercase'}}>{h}</th>)}</tr></thead>
            <tbody>
              {filtered.map(item=>{
                const isLow = item.current_stock <= item.minimum_stock
                const value = Number(item.current_stock) * Number(item.unit_price)
                return (
                  <tr key={item.item_id} style={{borderBottom:'1px solid #0f172a',background:isLow?'#ef444408':'transparent'}}>
                    <td style={{padding:'10px 8px',color:'#f1f5f9',fontWeight:600,fontSize:13}}>{item.item_name}</td>
                    <td style={{padding:'10px 8px',color:'#64748b',fontSize:11}}>{item.item_code}</td>
                    <td style={{padding:'10px 8px',color:'#94a3b8',fontSize:12}}>{item.category}</td>
                    ...
                    <td style={{padding:'10px 8px',color:'#94a3b8',fontSize:12}}>{item.supplier}</td>
                    ...
                    <td style={{padding:'10px 8px'}}>
                      <div style={{display:'flex',gap:4}}>
                        ...
                        <button onClick={()=>{setEditItem(item);setForm(item);setShowForm(true)}} style={{background:'#334155',border:'none',borderRadius:6,color:'#94a3b8',padding:'4px 8px',cursor:'pointer',fontSize:11}}><Edit size={10}/></button>
                      </div>
                    </td>
                  </tr>
                )
      ...
      {showForm && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={()=>setShowForm(false)}>
          <div style={{background:'#1e293b',border:'1px solid #334155',borderRadius:16,padding:24,width:560}} onClick={e=>e.stopPropagation()}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}><h2 style={{color:'#f1f5f9',margin:0}}>{editItem?'Edit Item':'Add Item'}</h2><button onClick={()=>setShowForm(false)} style={{background:'none',border:'none',color:'#94a3b8',cursor:'pointer'}}><X size={20}/></button></div>
      ...
      {showRestock && selectedItem && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center'}} onClick={()=>setShowRestock(false)}>
          <div style={{background:'#1e293b',border:'1px solid #334155',borderRadius:16,padding:24,width:400}} onClick={e=>e.stopPropagation()}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}><h2 style={{color:'#f1f5f9',margin:0}}>Restock — {selectedItem.item_name}</h2><button onClick={()=>setShowRestock(false)} style={{background:'none',border:'none',color:'#94a3b8',cursor:'pointer'}}><X size={20}/></button></div>
            <div style={{background:'#0f172a',borderRadius:8,padding:12,marginBottom:16,fontSize:13,color:'#94a3b8'}}>Current Stock: <strong style={{color:'#f1f5f9'}}>{selectedItem.current_stock} {selectedItem.unit}</strong></div>
            <div style={{display:'grid',gap:12}}>
              <div><label style={{fontSize:11,color:'#64748b',display:'block',marginBottom:4}}>Quantity to Add</label><input type="number" value={restockForm.quantity} onChange={e=>setRestockForm({...restockForm,quantity:e.target.value})} style={{width:'100%',background:'#0f172a',border:'1px solid #334155',borderRadius:8,color:'#f1f5f9',padding:'8px 12px',fontSize:13,boxSizing:'border-box'}}/></div>
              <div><label style={{fontSize:11,color:'#64748b',display:'block',marginBottom:4}}>PO Reference</label><input value={restockForm.reference} onChange={e=>setRestockForm({...restockForm,reference:e.target.value})} style={{width:'100%',background:'#0f172a',border:'1px solid #334155',borderRadius:8,color:'#f1f5f9',padding:'8px 12px',fontSize:13,boxSizing:'border-box'}}/></div>
              <div><label style={{fontSize:11,color:'#64748b',display:'block',marginBottom:4}}>Notes</label><textarea value={restockForm.notes} onChange={e=>setRestockForm({...restockForm,notes:e.target.value})} rows={2} style={{width:'100%',background:'#0f172a',border:'1px solid #334155',borderRadius:8,color:'#f1f5f9',padding:'8px 12px',fontSize:13,boxSizing:'border-box',resize:'vertical'}}/></div>
            </div>
            <div style={{display:'flex',gap:12,marginTop:20}}>
              <button onClick={()=>setShowRestock(false)} style={{flex:1,background:'#334155',border:'none',borderRadius:8,color:'#94a3b8',padding:'10px',cursor:'pointer'}}>Cancel</button>
