      ...
      <div style={{background:'#ffffff',borderBottom:'1px solid #e2e8f0',padding:'16px 24px',display:'flex',alignItems:'center',gap:16}}>
        ...
        <div style={{marginLeft:'auto',display:'flex',gap:10}}>
          <div style={{display:'flex',gap:4,background:'#f8fafc',padding:4,borderRadius:8}}>
            {['kanban','list'].map(v=><button key={v} onClick={()=>setView(v)} style={{padding:'6px 12px',borderRadius:6,border:'none',background:view===v?'#e2e8f0':'transparent',color:view===v?'#0f172a':'#64748b',cursor:'pointer',fontSize:12,textTransform:'capitalize'}}>{v}</button>)}
          </div>
      ...
      <div style={{padding:24}}>
        ...
        {stats?.byStage?.length > 0 && (
          <div style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:12,padding:20,marginBottom:20}}>
            <h3 style={{color:'#0f172a',marginBottom:16,fontSize:14}}>Pipeline by Stage (₹ Lakhs)</h3>
        ...
        {view==='kanban' && (
          <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:8,overflowX:'auto'}}>
            {STAGES.map(stage=>(
              <div key={stage} style={{background:'#ffffff',borderRadius:10,padding:10,minHeight:300}}>
                ...
                {deals.filter(d=>d.stage===stage).map(deal=>(
                  <div key={deal.deal_id} style={{background:'#f8fafc',borderRadius:8,padding:10,marginBottom:8,border:`1px solid ${STAGE_COLOR[stage]}30`}}>
                    <div style={{fontSize:12,color:'#0f172a',fontWeight:600,marginBottom:4}}>{deal.deal_name}</div>
                    ...
                    <div style={{display:'flex',gap:4,flexWrap:'wrap'}}>
                      {stage!=='Closed Won' && stage!=='Closed Lost' && (
                        <button onClick={()=>handleStageUpdate(deal, STAGES[STAGES.indexOf(stage)+1])} style={{background:'#e2e8f0',border:'none',borderRadius:4,color:'#475569',padding:'2px 6px',cursor:'pointer',fontSize:9}}>→ Next</button>
                      )}
                      <button onClick={()=>{setEditDeal(deal);setForm(deal);setShowForm(true)}} style={{background:'#e2e8f0',border:'none',borderRadius:4,color:'#475569',padding:'2px 6px',cursor:'pointer',fontSize:9}}><Edit size={8}/></button>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {view==='list' && (
          <div style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:12,padding:20}}>
            <table style={{width:'100%',borderCollapse:'collapse'}}>
              <thead><tr style={{borderBottom:'1px solid #e2e8f0'}}>{['Deal','Company','Contact','Value','Stage','Probability','Close Date','Assigned To',''].map(h=><th key={h} style={{color:'#64748b',fontSize:11,padding:'8px',textAlign:'left',textTransform:'uppercase'}}>{h}</th>)}</tr></thead>
              <tbody>
                {deals.map(d=>(
                  <tr key={d.deal_id} style={{borderBottom:'1px solid #f1f5f9'}}>
                    <td style={{padding:'10px 8px',color:'#0f172a',fontWeight:600,fontSize:13}}>{d.deal_name}</td>
                    <td style={{padding:'10px 8px',color:'#475569',fontSize:12}}>{d.company_name}</td>
                    <td style={{padding:'10px 8px',color:'#475569',fontSize:12}}>{d.contact_name}</td>
                    ...
                    <td style={{padding:'10px 8px',color:'#475569',fontSize:12}}>{d.expected_close?.slice(0,10)}</td>
                    <td style={{padding:'10px 8px',color:'#475569',fontSize:12}}>{d.assigned_to}</td>
                    <td style={{padding:'10px 8px'}}><button onClick={()=>{setEditDeal(d);setForm(d);setShowForm(true)}} style={{background:'#e2e8f0',border:'none',borderRadius:6,color:'#475569',padding:'4px 8px',cursor:'pointer',fontSize:12}}><Edit size={12}/></button></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {showForm && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={()=>setShowForm(false)}>
          <div style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:16,padding:24,width:560}} onClick={e=>e.stopPropagation()}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}><h2 style={{color:'#0f172a',margin:0}}>{editDeal?'Edit Deal':'New Deal'}</h2><button onClick={()=>setShowForm(false)} style={{background:'none',border:'none',color:'#475569',cursor:'pointer'}}><X size={20}/></button></div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
              ...
              <div><label style={{fontSize:11,color:'#64748b',display:'block',marginBottom:4}}>Stage</label><select value={form.stage} onChange={e=>setForm({...form,stage:e.target.value})} style={{width:'100%',background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:8,color:'#0f172a',padding:'8px 12px',fontSize:13}}>{STAGES.map(s=><option key={s}>{s}</option>)}</select></div>
              <div><label style={{fontSize:11,color:'#64748b',display:'block',marginBottom:4}}>Source</label><select value={form.source} onChange={e=>setForm({...form,source:e.target.value})} style={{width:'100%',background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:8,color:'#0f172a',padding:'8px 12px',fontSize:13}}>{SOURCES.map(s=><option key={s}>{s}</option>)}</select></div>
            </div>
