        ...
        {tab==='overview' && (
          <div>
            ...
            <div style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:12,padding:20,marginBottom:24}}>
              <h3 style={{color:'#1e293b',marginBottom:16,fontSize:14}}>Budget vs Spent (₹ Lakhs)</h3>
            ...
            <div style={{display:'flex',gap:12,marginBottom:16,alignItems:'center'}}>
              <div style={{display:'flex',alignItems:'center',gap:8,background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:8,padding:'8px 12px',flex:1}}>
                ...
                <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search projects..." style={{background:'none',border:'none',color:'#1e293b',fontSize:13,outline:'none',flex:1}}/>
              </div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:16}}>
              {filtered.map(p=>(
                <div key={p.project_id} onClick={()=>loadProjectDetail(p)} style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:12,padding:20,cursor:'pointer',transition:'border-color 0.2s'}} onMouseEnter={e=>e.currentTarget.style.borderColor='#3b82f6'} onMouseLeave={e=>e.currentTarget.style.borderColor='#334155'}>
                  ...
                  <h3 style={{color:'#1e293b',margin:'0 0 4px',fontSize:15}}>{p.project_name}</h3>
        ...
        {tab==='detail' && selectedProject && (
          <div>
            ...
            <div style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:12,padding:16,marginBottom:24,display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:16}}>
              {[
                ...
                <div key={idx}>
                  ...
                  <div style={{fontSize:13,color:'#1e293b',fontWeight:600}}>{i.value}</div>
                </div>
              ))}
            </div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'2fr 1fr',gap:20}}>
              {/* Kanban Board */}
              <div>
                <h3 style={{color:'#1e293b',marginBottom:16,fontSize:14}}>Task Board</h3>
                    ...
                    <div key={col} style={{background:'#ffffff',borderRadius:10,padding:12,minHeight:200}}>
                      ...
                      {tasks.filter(t=>t.status===col).map(task=>(
                        <div key={task.task_id} style={{background:'#f8fafc',borderRadius:8,padding:10,marginBottom:8,border:'1px solid #e2e8f0'}}>
                          <div style={{fontSize:12,color:'#1e293b',fontWeight:600,marginBottom:4}}>{task.task_name}</div>
              ...
              {/* Milestones */}
              <div>
                <h3 style={{color:'#1e293b',marginBottom:16,fontSize:14}}>Milestones</h3>
                ...
                <div style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:10,padding:16,marginTop:16}}>
                  <h4 style={{color:'#1e293b',margin:'0 0 12px',fontSize:13}}>Task Summary</h4>
                  {TASK_COLUMNS.map(col=>(
                    <div key={col} style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
                      ...
                      <span style={{fontSize:13,fontWeight:600,color:'#1e293b'}}>{tasks.filter(t=>t.status===col).length}</span>
                    </div>
                  ))}
            ...
            {detailTab==='gantt' && (
              <div style={{background:"#ffffff",border:"1px solid #e2e8f0",borderRadius:12,padding:20}}>
                <h3 style={{color:"#1e293b",marginBottom:20,fontSize:14}}>Project Timeline (Gantt Chart)</h3>
                    ...
                    {tasks.map((task,i)=>{
                      ...
                      return (
                        <div key={i} style={{display:"grid",gridTemplateColumns:"200px 1fr",gap:0,marginBottom:4,alignItems:"center"}}>
                          <div style={{fontSize:12,color:"#1e293b",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",paddingRight:8}}>{task.task_name}</div>
            ...
            {detailTab==="team" && (
              <div style={{background:"#ffffff",border:"1px solid #e2e8f0",borderRadius:12,padding:20}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
                  <h3 style={{color:"#1e293b",margin:0,fontSize:14}}>Team Members ({teamMembers.length})</h3>
                  ...
                  <div style={{background:"#f8fafc",borderRadius:10,padding:16,marginBottom:16,display:"grid",gridTemplateColumns:"1fr 1fr 1fr auto",gap:8,alignItems:"end"}}>
                    {[["name","Name"],["role","Role"],["email","Email"]].map(([key,label])=>(
                      <div key={key}>
                        ...
                        <input value={teamForm[key]||""} onChange={e=>setTeamForm({...teamForm,[key]:e.target.value})} style={{width:"100%",background:"#ffffff",border:"1px solid #e2e8f0",borderRadius:6,color:"#1e293b",padding:"6px 10px",fontSize:12,boxSizing:"border-box"}}/>
                      </div>
                    ))}
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12}}>
                  {teamMembers.map(m=>(
                    <div key={m.member_id} style={{background:"#f8fafc",borderRadius:10,padding:16,display:"flex",alignItems:"center",gap:12}}>
                      ...
                      <div style={{flex:1}}>
                        <div style={{fontSize:13,color:"#1e293b",fontWeight:600}}>{m.name}</div>
            ...
            {detailTab==="budget" && (
              <div style={{background:"#ffffff",border:"1px solid #e2e8f0",borderRadius:12,padding:20}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
                  <h3 style={{color:"#1e293b",margin:0,fontSize:14}}>Budget Tracking</h3>
                ...
                {showBudgetForm && (
                  <div style={{background:"#f8fafc",borderRadius:10,padding:16,marginBottom:16,display:"grid",gridTemplateColumns:"1fr 1fr auto",gap:8,alignItems:"end"}}>
                    <div>
                      ...
                      <input type="number" value={budgetForm.spent} onChange={e=>setBudgetForm({...budgetForm,spent:e.target.value})} style={{width:"100%",background:"#ffffff",border:"1px solid #e2e8f0",borderRadius:6,color:"#1e293b",padding:"6px 10px",fontSize:12,boxSizing:"border-box"}}/>
                    </div>
                    <div>
                      <label style={{fontSize:10,color:"#64748b",display:"block",marginBottom:4}}>Progress %</label>
                      <input type="number" min="0" max="100" value={budgetForm.progress} onChange={e=>setBudgetForm({...budgetForm,progress:e.target.value})} style={{width:"100%",background:"#ffffff",border:"1px solid #e2e8f0",borderRadius:6,color:"#1e293b",padding:"6px 10px",fontSize:12,boxSizing:"border-box"}}/>
                    </div>
            ...
            {detailTab==="comments" && (
              <div style={{background:"#ffffff",border:"1px solid #e2e8f0",borderRadius:12,padding:20}}>
                <h3 style={{color:"#1e293b",marginBottom:16,fontSize:14}}>Activity Log & Comments ({comments.length})</h3>
                <div style={{display:"flex",gap:8,marginBottom:20}}>
                  <input value={newComment} onChange={e=>setNewComment(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleAddComment()} placeholder="Add a comment or update..." style={{flex:1,background:"#f8fafc",border:"1px solid #e2e8f0",borderRadius:8,color:"#1e293b",padding:"10px 14px",fontSize:13}}/>
                    ...
                    <div key={c.comment_id} style={{background:"#f8fafc",borderRadius:10,padding:16,marginBottom:12,display:"flex",gap:12}}>
                      ...
                      <div style={{flex:1}}>
                        <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
                          <span style={{fontSize:13,color:"#1e293b",fontWeight:600}}>{c.author}</span>
            ...
            {detailTab==="files" && (
              <div style={{background:"#ffffff",border:"1px solid #e2e8f0",borderRadius:12,padding:20}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
                  <h3 style={{color:"#1e293b",margin:0,fontSize:14}}>File Attachments ({attachments.length})</h3>
                  ...
                  <div style={{display:"grid",gap:8}}>
                    {attachments.map(a=>(
                      <div key={a.attachment_id} style={{background:"#f8fafc",borderRadius:8,padding:12,display:"flex",alignItems:"center",gap:12}}>
                        ...
                        <div style={{flex:1}}>
                          <div style={{fontSize:13,color:"#1e293b",fontWeight:600}}>{a.file_name}</div>
      ...
      {showPrjForm && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={()=>setShowPrjForm(false)}>
          <div style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:16,padding:24,width:580,maxHeight:'90vh',overflowY:'auto'}} onClick={e=>e.stopPropagation()}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
              <h2 style={{color:'#1e293b',margin:0}}>{editPrj?'Edit Project':'New Project'}</h2>
      ...
      {showTaskForm && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={()=>setShowTaskForm(false)}>
          <div style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:16,padding:24,width:480}} onClick={e=>e.stopPropagation()}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
              <h2 style={{color:'#1e293b',margin:0}}>{editTask?'Edit Task':'Add Task'}</h2>
            ...
            <div style={{display:'grid',gap:12}}>
              {[['task_name','Task Name'],['assigned_to','Assigned To'],['due_date','Due Date','date'],['estimated_hours','Estimated Hours','number']].map(([key,label,type='text'])=>(
                <div key={key}>
                  <label style={{fontSize:11,color:'#64748b',display:'block',marginBottom:4}}>{label}</label>
                  <input type={type} value={taskForm[key]||''} onChange={e=>setTaskForm({...taskForm,[key]:e.target.value})} style={{width:'100%',background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:8,color:'#1e293b',padding:'8px 12px',fontSize:13,boxSizing:'border-box'}}/>
                </div>
              ))}
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
                <div>
                  <label style={{fontSize:11,color:'#64748b',display:'block',marginBottom:4}}>Status</label>
                  <select value={taskForm.status} onChange={e=>setTaskForm({...taskForm,status:e.target.value})} style={{width:'100%',background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:8,color:'#1e293b',padding:'8px 12px',fontSize:13}}>
                    {TASK_COLUMNS.map(s=><option key={s}>{s}</option>)}
                  </select>
                </div>
                <div>
                  <label style={{fontSize:11,color:'#64748b',display:'block',marginBottom:4}}>Priority</label>
                  <select value={taskForm.priority} onChange={e=>setTaskForm({...taskForm,priority:e.target.value})} style={{width:'100%',background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:8,color:'#1e293b',padding:'8px 12px',fontSize:13}}>
                    {['High','Medium','Low'].map(p=><option key={p}>{p}</option>)}
                  </select>
                </div>
              </div>
              <div>
                <label style={{fontSize:11,color:'#64748b',display:'block',marginBottom:4}}>Description</label>
                <textarea value={taskForm.description||''} onChange={e=>setTaskForm({...taskForm,description:e.target.value})} rows={2} style={{width:'100%',background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:8,color:'#1e293b',padding:'8px 12px',fontSize:13,boxSizing:'border-box',resize:'vertical'}}/>
              </div>
            </div>
            <div style={{display:'flex',gap:12,marginTop:20}}>
