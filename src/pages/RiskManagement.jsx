    ...
    <div style={{minHeight:'100vh',background:'#f8fafc',color:'#0f172a',fontFamily:'Inter,sans-serif'}}>
      ...
      <div style={{padding:24}}>
        ...
        <div style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:12,padding:20,marginBottom:20}}>
          <h3 style={{color:'#0f172a',marginBottom:16,fontSize:14}}>Risk Heatmap</h3>
        ...
        <div style={{display:'flex',gap:12,marginBottom:16}}>
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search risks..." style={{flex:1,background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:8,color:'#0f172a',padding:'10px 14px',fontSize:13}}/>
        ...
        <div style={{display:'grid',gap:12}}>
          {filtered.map(r=>{
            ...
            return (
              <div key={r.risk_id} style={{background:'#ffffff',border:`1px solid ${level.color}30`,borderLeft:`4px solid ${level.color}`,borderRadius:12,padding:20}}>
                <div style={{display:'flex',justifyContent:'space-between',marginBottom:12}}>
                  <div style={{display:'flex',gap:10,alignItems:'center'}}>
                    <AlertTriangle size={16} style={{color:level.color}}/>
                    <div>
                      <div style={{color:'#0f172a',fontWeight:700,fontSize:14}}>{r.risk_name}</div>
                  ...
                  <div style={{display:'flex',gap:8,alignItems:'center'}}>
                    ...
                    <button onClick={()=>{setEditRisk(r);setForm(r);setShowForm(true)}} style={{background:'#e2e8f0',border:'none',borderRadius:6,color:'#475569',padding:'6px 8px',cursor:'pointer'}}><Edit size={12}/></button>
                  </div>
                </div>
                <p style={{color:'#475569',fontSize:13,margin:'0 0 12px'}}>{r.description}</p>
                <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:8,fontSize:12}}>
                  <div><span style={{color:'#64748b'}}>Likelihood: </span><span style={{color:'#0f172a',fontWeight:600}}>{r.likelihood}/5</span></div>
                  <div><span style={{color:'#64748b'}}>Impact: </span><span style={{color:'#0f172a',fontWeight:600}}>{r.impact}/5</span></div>
                  <div><span style={{color:'#64748b'}}>Owner: </span><span style={{color:'#475569'}}>{r.owner}</span></div>
                  <div><span style={{color:'#64748b'}}>Due: </span><span style={{color:'#475569'}}>{r.due_date?.slice(0,10)}</span></div>
                </div>
                {r.mitigation && <div style={{background:'#f8fafc',borderRadius:8,padding:10,marginTop:10,fontSize:12,color:'#475569'}}><span style={{color:'#10b981',fontWeight:600}}>Mitigation: </span>{r.mitigation}</div>}
      ...
      {showForm && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={()=>setShowForm(false)}>
          <div style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:16,padding:24,width:560,maxHeight:'90vh',overflowY:'auto'}} onClick={e=>e.stopPropagation()}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}><h2 style={{color:'#0f172a',margin:0}}>{editRisk?'Edit Risk':'Add Risk'}</h2><button onClick={()=>setShowForm(false)} style={{background:'none',border:'none',color:'#475569',cursor:'pointer'}}><X size={20}/></button></div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
              ...
              <div><label style={{fontSize:11,color:'#64748b',display:'block',marginBottom:4}}>Likelihood (1-5)</label><input type="range" min="1" max="5" value={form.likelihood} onChange={e=>setForm({...form,likelihood:Number(e.target.value)})} style={{width:'100%'}}/><div style={{textAlign:'center',color:'#0f172a',fontSize:13,fontWeight:600}}>{form.likelihood}</div></div>
              ...
              <div style={{gridColumn:'span 2'}}><label style={{fontSize:11,color:'#64748b',display:'block',marginBottom:4}}>Mitigation Plan</label><textarea value={form.mitigation||''} onChange={e=>setForm({...form,mitigation:e.target.value})} rows={2} style={{width:'100%',background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:8,color:'#0f172a',padding:'8px 12px',fontSize:13,boxSizing:'border-box',resize:'vertical'}}/></div>
            </div>
