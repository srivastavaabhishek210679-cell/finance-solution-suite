    ...
    <div style={{minHeight:'100vh',background:'#0f172a',color:'#f1f5f9',fontFamily:'Inter,sans-serif'}}>
      ...
      <div style={{padding:24}}>
        ...
        {stats?.byCategory?.length > 0 && (
          <div style={{display:'flex',gap:8,marginBottom:20,flexWrap:'wrap'}}>
            {stats.byCategory.map(c=>(
              <div key={c.category} style={{background:'#1e293b',border:'1px solid #334155',borderRadius:8,padding:'8px 14px',display:'flex',gap:8,alignItems:'center',cursor:'pointer'}} onClick={()=>setFilterCat(filterCat===c.category?'All':c.category)}>
                <span style={{fontSize:12,color:'#94a3b8'}}>{c.category}:</span>
        ...
        <div style={{display:'flex',gap:12,marginBottom:16,flexWrap:'wrap'}}>
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search compliance items..." style={{flex:1,minWidth:180,background:'#1e293b',border:'1px solid #334155',borderRadius:8,color:'#f1f5f9',padding:'10px 14px',fontSize:13}}/>
        ...
        <div style={{display:'grid',gap:12}}>
          {filtered.map(item=>{
            ...
            return (
              <div key={item.compliance_id} style={{background:'#1e293b',border:`1px solid ${isOverdue?'#ef444440':isUrgent?'#f59e0b40':'#334155'}`,borderLeft:`4px solid ${isOverdue?'#ef4444':isUrgent?'#f59e0b':PRIORITY_COLOR[item.priority]}`,borderRadius:12,padding:20}}>
                <div style={{display:'flex',justifyContent:'space-between',marginBottom:10}}>
                  <div style={{display:'flex',gap:10,alignItems:'center'}}>
                    ...
                    <div>
                      <div style={{color:'#f1f5f9',fontWeight:700,fontSize:14}}>{item.title}</div>
                  ...
                  <div style={{display:'flex',gap:6,alignItems:'center'}}>
                    ...
                    <button onClick={()=>{setEditItem(item);setForm(item);setShowForm(true)}} style={{background:'#334155',border:'none',borderRadius:6,color:'#94a3b8',padding:'4px 8px',cursor:'pointer'}}><Edit size={12}/></button>
                  </div>
                </div>
                {item.description && <p style={{color:'#94a3b8',fontSize:12,margin:'0 0 10px'}}>{item.description}</p>}
                <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:8,fontSize:12}}>
                  <div><span style={{color:'#64748b'}}>Owner: </span><span style={{color:'#94a3b8'}}>{item.owner}</span></div>
                  <div><span style={{color:'#64748b'}}>Due: </span><span style={{color:isOverdue?'#ef4444':isUrgent?'#f59e0b':'#94a3b8'}}>{item.due_date?.slice(0,10)}</span></div>
      ...
      {showForm && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={()=>setShowForm(false)}>
          <div style={{background:'#1e293b',border:'1px solid #334155',borderRadius:16,padding:24,width:560,maxHeight:'90vh',overflowY:'auto'}} onClick={e=>e.stopPropagation()}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}><h2 style={{color:'#f1f5f9',margin:0}}>{editItem?'Edit Item':'Add Compliance Item'}</h2><button onClick={()=>setShowForm(false)} style={{background:'none',border:'none',color:'#94a3b8',cursor:'pointer'}}><X size={20}/></button></div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
              ...
              <div><label style={{fontSize:11,color:'#64748b',display:'block',marginBottom:4}}>Priority</label><select value={form.priority} onChange={e=>setForm({...form,priority:e.target.value})} style={{width:'100%',background:'#0f172a',border:'1px solid #334155',borderRadius:8,color:'#f1f5f9',padding:'8px 12px',fontSize:13}}>{['High','Medium','Low'].map(p=><option key={p}>{p}</option>)}</select></div>
              {editItem && <div><label style={{fontSize:11,color:'#64748b',display:'block',marginBottom:4}}>Status</label><select value={form.status||'Pending'} onChange={e=>setForm({...form,status:e.target.value})} style={{width:'100%',background:'#0f172a',border:'1px solid #334155',borderRadius:8,color:'#f1f5f9',padding:'8px 12px',fontSize:13}}>{['Pending','In Progress','Completed'].map(s=><option key={s}>{s}</option>)}</select></div>}
