    ...
    <div style={{minHeight:'100vh',background:'#f1f5f9',color:'#1e293b',fontFamily:'Inter,sans-serif'}}>
      ...
      <div style={{padding:24}}>
        ...
        <div style={{display:'flex',gap:12,marginBottom:16,flexWrap:'wrap'}}>
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search compliance items..." style={{flex:1,minWidth:180,background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:8,color:'#1e293b',padding:'10px 14px',fontSize:13}}/>
        ...
        <div style={{display:'grid',gap:12}}>
          {filtered.map(item=>{
            ...
            return (
              <div key={item.compliance_id} style={{background:'#ffffff',border:`1px solid ${isOverdue?'#ef444440':isUrgent?'#f59e0b40':'#334155'}`,borderLeft:`4px solid ${isOverdue?'#ef4444':isUrgent?'#f59e0b':PRIORITY_COLOR[item.priority]}`,borderRadius:12,padding:20}}>
                <div style={{display:'flex',justifyContent:'space-between',marginBottom:10}}>
                  <div style={{display:'flex',gap:10,alignItems:'center'}}>
                    ...
                    <div>
                      <div style={{color:'#1e293b',fontWeight:700,fontSize:14}}>{item.title}</div>
      ...
      {showForm && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={()=>setShowForm(false)}>
          <div style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:16,padding:24,width:560,maxHeight:'90vh',overflowY:'auto'}} onClick={e=>e.stopPropagation()}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}><h2 style={{color:'#1e293b',margin:0}}>{editItem?'Edit Item':'Add Compliance Item'}</h2><button onClick={()=>setShowForm(false)} style={{background:'none',border:'none',color:'#475569',cursor:'pointer'}}><X size={20}/></button></div>
