  ...
  return (
    <div style={{minHeight:'100vh',background:'#f8fafc',color:'#0f172a',fontFamily:'Inter,sans-serif'}}>
      ...
      <div style={{background:'#ffffff',borderBottom:'1px solid #e2e8f0',padding:'16px 24px',display:'flex',alignItems:'center',gap:16}}>
        <button onClick={()=>selectedJob?setSelectedJob(null):navigate('/dashboard')} style={{background:'#e2e8f0',border:'none',borderRadius:8,color:'#475569',padding:'8px 12px',cursor:'pointer',display:'flex',alignItems:'center',gap:6}}><ArrowLeft size={16}/> {selectedJob?'Jobs':'Back'}</button>
      ...
      <div style={{padding:24}}>
        ...
        {!selectedJob && pipeline.length > 0 && (
          <div style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:12,padding:20,marginBottom:20}}>
            <h3 style={{color:'#0f172a',marginBottom:16,fontSize:14}}>Recruitment Pipeline</h3>
            <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
              {pipeline.map((p,i)=>(
                <div key={p.stage} style={{display:'flex',alignItems:'center',gap:8}}>
                  ...
                  {i < pipeline.length-1 && <ChevronRight size={16} style={{color:'#e2e8f0'}}/>}
                </div>
              ))}
            </div>
          </div>
        ...
        {!selectedJob && (
          <div>
            <div style={{display:'flex',gap:4,marginBottom:16,background:'#ffffff',padding:4,borderRadius:10,width:'fit-content'}}>
            ...
            {tab==='jobs' && (
              <div>
                <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search jobs..." style={{width:'100%',background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:8,color:'#0f172a',padding:'10px 14px',fontSize:13,marginBottom:16,boxSizing:'border-box'}}/>
                <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:16}}>
                  {filteredJobs.map(j=>(
                    <div key={j.job_id} onClick={()=>setSelectedJob(j)} style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:12,padding:20,cursor:'pointer',transition:'border-color 0.2s'}} onMouseEnter={e=>e.currentTarget.style.borderColor='#8b5cf6'} onMouseLeave={e=>e.currentTarget.style.borderColor='#e2e8f0'}>
                      ...
                      <div style={{color:'#0f172a',fontWeight:700,fontSize:15,marginBottom:4}}>{j.job_title}</div>
                      ...
                      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6,fontSize:12,marginBottom:10}}>
                        <div><span style={{color:'#64748b'}}>Exp: </span><span style={{color:'#475569'}}>{j.experience}</span></div>
            ...
            {tab==='pipeline' && (
              <div style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:12,padding:20}}>
                <table style={{width:'100%',borderCollapse:'collapse'}}>
                  <thead><tr style={{borderBottom:'1px solid #e2e8f0'}}>{['Candidate','Job','Company','Experience','Status','Actions'].map(h=><th key={h} style={{color:'#64748b',fontSize:11,padding:'8px',textAlign:'left',textTransform:'uppercase'}}>{h}</th>)}</tr></thead>
                  <tbody>
                    {applications.map(a=>(
                      <tr key={a.application_id} style={{borderBottom:'1px solid #f1f5f9'}}>
                        <td style={{padding:'10px 8px'}}>
                          <div style={{color:'#0f172a',fontWeight:600,fontSize:13}}>{a.candidate_name}</div>
                          <div style={{color:'#64748b',fontSize:11}}>{a.email}</div>
                        </td>
                        <td style={{padding:'10px 8px',color:'#475569',fontSize:12}}>{a.job_title}</td>
                        <td style={{padding:'10px 8px',color:'#475569',fontSize:12}}>{a.current_company}</td>
                        ...
                        <td style={{padding:'10px 8px'}}>
                          <select onChange={e=>handleUpdateStatus(a.application_id,e.target.value)} value={a.status} style={{background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:6,color:'#0f172a',padding:'4px 8px',fontSize:11,cursor:'pointer'}}>
        ...
        {selectedJob && (
          <div>
            <div style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:12,padding:16,marginBottom:20,display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:12}}>
              {[['Type',selectedJob.job_type],['Experience',selectedJob.experience],['Salary',selectedJob.salary_range],['Openings',selectedJob.openings],['Closes',selectedJob.closing_date?.slice(0,10)]].map(([l,v],i)=>(
                <div key={i}><div style={{fontSize:10,color:'#64748b',marginBottom:4}}>{l}</div><div style={{fontSize:13,color:'#0f172a',fontWeight:600}}>{v}</div></div>
              ))}
            </div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:8,marginBottom:20}}>
              {APP_STAGES.map(stage=>{
                ...
                return (
                  <div key={stage} style={{background:'#ffffff',borderRadius:8,padding:10,textAlign:'center',border:`1px solid ${count>0?STAGE_COLORS[stage]+'40':'#e2e8f0'}`}}>
                    <div style={{fontSize:18,fontWeight:700,color:count>0?STAGE_COLORS[stage]:'#e2e8f0'}}>{count}</div>
            ...
            <div style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:12,padding:20}}>
              <h3 style={{color:'#0f172a',margin:'0 0 16px',fontSize:14}}>Candidates ({jobApps.length})</h3>
              {jobApps.length===0 ? <div style={{textAlign:'center',padding:40,color:'#64748b'}}>No applications yet</div> : (
                <div style={{display:'grid',gap:10}}>
                  {jobApps.map(a=>(
                    <div key={a.application_id} style={{background:'#f8fafc',borderRadius:10,padding:16,display:'flex',alignItems:'center',gap:16}}>
                      ...
                      <div style={{flex:1}}>
                        <div style={{color:'#0f172a',fontWeight:600,fontSize:14}}>{a.candidate_name}</div>
                      ...
                      <select onChange={e=>handleUpdateStatus(a.application_id,e.target.value)} value={a.status} style={{background:'#ffffff',border:`1px solid ${STAGE_COLORS[a.status]||'#e2e8f0'}`,borderRadius:8,color:STAGE_COLORS[a.status]||'#475569',padding:'6px 12px',fontSize:12,cursor:'pointer',fontWeight:600}}>
                        {APP_STAGES.map(s=><option key={s}>{s}</option>)}
                      </select>
                    </div>
                  ))}
      ...
      {showJobForm && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={()=>setShowJobForm(false)}>
          <div style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:16,padding:24,width:560,maxHeight:'90vh',overflowY:'auto'}} onClick={e=>e.stopPropagation()}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}><h2 style={{color:'#0f172a',margin:0}}>Post New Job</h2><button onClick={()=>setShowJobForm(false)} style={{background:'none',border:'none',color:'#475569',cursor:'pointer'}}><X size={20}/></button></div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
              {[['job_title','Job Title'],['location','Location'],['experience','Experience Required'],['salary_range','Salary Range'],['openings','No. of Openings','number'],['closing_date','Closing Date','date']].map(([key,label,type='text'])=>(
                <div key={key}><label style={{fontSize:11,color:'#64748b',display:'block',marginBottom:4}}>{label}</label><input type={type} value={jobForm[key]||''} onChange={e=>setJobForm({...jobForm,[key]:e.target.value})} style={{width:'100%',background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:8,color:'#0f172a',padding:'8px 12px',fontSize:13,boxSizing:'border-box'}}/></div>
              ))}
              <div><label style={{fontSize:11,color:'#64748b',display:'block',marginBottom:4}}>Department</label><select value={jobForm.department} onChange={e=>setJobForm({...jobForm,department:e.target.value})} style={{width:'100%',background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:8,color:'#0f172a',padding:'8px 12px',fontSize:13}}>{DEPTS.map(d=><option key={d}>{d}</option>)}</select></div>
              <div><label style={{fontSize:11,color:'#64748b',display:'block',marginBottom:4}}>Job Type</label><select value={jobForm.job_type} onChange={e=>setJobForm({...jobForm,job_type:e.target.value})} style={{width:'100%',background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:8,color:'#0f172a',padding:'8px 12px',fontSize:13}}>{JOB_TYPES.map(t=><option key={t}>{t}</option>)}</select></div>
              <div style={{gridColumn:'span 2'}}><label style={{fontSize:11,color:'#64748b',display:'block',marginBottom:4}}>Job Description</label><textarea value={jobForm.description||''} onChange={e=>setJobForm({...jobForm,description:e.target.value})} rows={3} style={{width:'100%',background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:8,color:'#0f172a',padding:'8px 12px',fontSize:13,boxSizing:'border-box',resize:'vertical'}}/></div>
            </div>
            <div style={{display:'flex',gap:12,marginTop:20}}>
              <button onClick={()=>setShowJobForm(false)} style={{flex:1,background:'#e2e8f0',border:'none',borderRadius:8,color:'#475569',padding:'10px',cursor:'pointer'}}>Cancel</button>
      ...
      {showAppForm && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center'}} onClick={()=>setShowAppForm(false)}>
          <div style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:16,padding:24,width:460}} onClick={e=>e.stopPropagation()}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}><h2 style={{color:'#0f172a',margin:0}}>Add Candidate</h2><button onClick={()=>setShowAppForm(false)} style={{background:'none',border:'none',color:'#475569',cursor:'pointer'}}><X size={20}/></button></div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
              {[['candidate_name','Full Name'],['email','Email'],['phone','Phone'],['current_company','Current Company'],['experience_years','Years of Experience','number']].map(([key,label,type='text'])=>(
                <div key={key}><label style={{fontSize:11,color:'#64748b',display:'block',marginBottom:4}}>{label}</label><input type={type} value={appForm[key]||''} onChange={e=>setAppForm({...appForm,[key]:e.target.value})} style={{width:'100%',background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:8,color:'#0f172a',padding:'8px 12px',fontSize:13,boxSizing:'border-box'}}/></div>
              ))}
              <div style={{gridColumn:'span 2'}}><label style={{fontSize:11,color:'#64748b',display:'block',marginBottom:4}}>Notes</label><textarea value={appForm.notes||''} onChange={e=>setAppForm({...appForm,notes:e.target.value})} rows={2} style={{width:'100%',background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:8,color:'#0f172a',padding:'8px 12px',fontSize:13,boxSizing:'border-box',resize:'vertical'}}/></div>
            </div>
            <div style={{display:'flex',gap:12,marginTop:20}}>
              <button onClick={()=>setShowAppForm(false)} style={{flex:1,background:'#e2e8f0',border:'none',borderRadius:8,color:'#475569',padding:'10px',cursor:'pointer'}}>Cancel</button>
