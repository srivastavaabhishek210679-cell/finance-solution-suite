        ...
        <div style={{display:'grid',gridTemplateColumns:'2fr 1fr',gap:20}}>
          <div>
            ...
            <div style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:12,padding:20,marginBottom:20}}>
              <h3 style={{color:'#1e293b',margin:'0 0 16px',fontSize:15,fontWeight:600}}>My Modules</h3>
                ...
                {selectedModules.map((path, i)=>{
                  ...
                  return (
                    <button key={i} onClick={()=>navigate(path)} style={{background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:8,padding:'12px 10px',cursor:'pointer',display:'flex',alignItems:'center',gap:8,transition:'border-color 0.2s'}} onMouseEnter={e=>e.currentTarget.style.borderColor='#10b981'} onMouseLeave={e=>e.currentTarget.style.borderColor='#334155'}>
                      ...
                      <span style={{color:'#1e293b',fontSize:11,textAlign:'left',lineHeight:1.3}}>{name}</span>
            ...
            <div style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:12,padding:20}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
                <h3 style={{color:'#1e293b',margin:0,fontSize:15,fontWeight:600}}>Recent Reports</h3>
                  ...
                  <tbody>
                    {reportHistory.map(r=>(
                      <tr key={r.history_id} style={{borderBottom:'1px solid #f1f5f9'}}>
                        <td style={{padding:'10px 8px',color:'#1e293b',fontSize:13,fontWeight:500}}>{r.report_name}</td>
          ...
          <div>
            <div style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:12,padding:20,marginBottom:20}}>
              <h3 style={{color:'#1e293b',margin:'0 0 16px',fontSize:15,fontWeight:600}}>🔍 Find a Report</h3>
              <div style={{display:'grid',gap:10}}>
                <input value={search} onChange={e=>setSearch(e.target.value)} onKeyDown={e=>e.key==='Enter'&&handleSearch()} placeholder="Report name, domain, date..." style={{width:'100%',background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:8,color:'#1e293b',padding:'10px 12px',fontSize:13,boxSizing:'border-box'}}/>
                <select value={domainFilter} onChange={e=>setDomainFilter(e.target.value)} style={{width:'100%',background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:8,color:'#1e293b',padding:'10px 12px',fontSize:13}}>
                    ...
                    <div style={{display:'grid',gap:6,maxHeight:300,overflowY:'auto'}}>
                      {searchResults.map(r=>(
                        <div key={r.history_id} style={{background:'#f8fafc',borderRadius:8,padding:10}}>
                          <div style={{color:'#1e293b',fontSize:13,fontWeight:500,marginBottom:4}}>{r.report_name}</div>
            ...
            {/* Quick Actions */}
            <div style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:12,padding:20}}>
              <h3 style={{color:'#1e293b',margin:'0 0 16px',fontSize:15,fontWeight:600}}>Quick Actions</h3>
              <div style={{display:'grid',gap:8}}>
                {[
                  ...
                  <button key={i} onClick={()=>navigate(a.path)} style={{background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:8,padding:'10px 14px',cursor:'pointer',display:'flex',alignItems:'center',gap:10,width:'100%',textAlign:'left'}} onMouseEnter={e=>e.currentTarget.style.borderColor=a.color} onMouseLeave={e=>e.currentTarget.style.borderColor='#334155'}>
                    <span style={{fontSize:16}}>{a.icon}</span>
                    <span style={{color:'#1e293b',fontSize:13}}>{a.label}</span>
