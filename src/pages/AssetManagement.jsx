    ...
    <div style={{minHeight:'100vh',background:'#f8fafc',color:'#0f172a',fontFamily:'Inter,sans-serif'}}>
      ...
      <div style={{padding:24}}>
          ...
          <div style={{background:'#ef444415',border:'1px solid #ef444440',borderRadius:12,padding:16,marginBottom:20}}>
            ...
            <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
              {stats.expiringWarranty.map(a=>(
                <span key={a.asset_id} style={{background:'#f8fafc',color:'#f59e0b',padding:'4px 10px',borderRadius:20,fontSize:12}}>{a.asset_name} — {a.warranty_expiry?.slice(0,10)}</span>
              ))}
            </div>
          </div>
        )}
        ...
        <div style={{display:'flex',gap:12,marginBottom:16,alignItems:'center'}}>
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search assets..." style={{flex:1,background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:8,color:'#0f172a',padding:'10px 14px',fontSize:13}}/>
        ...
        <div style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:12,padding:20}}>
          <h3 style={{color:'#0f172a',margin:'0 0 16px',fontSize:14}}>Assets ({filtered.length})</h3>
          <table style={{width:'100%',borderCollapse:'collapse'}}>
            <thead><tr style={{borderBottom:'1px solid #e2e8f0'}}>{['Asset','Code','Category','Department','Assigned To','Purchase Price','Current Value','Depreciation','Status',''].map(h=><th key={h} style={{color:'#64748b',fontSize:11,padding:'8px',textAlign:'left',textTransform:'uppercase'}}>{h}</th>)}</tr></thead>
            <tbody>
              {filtered.map(a=>{
                const depreciation = Math.round(((Number(a.purchase_price)-Number(a.current_value))/Number(a.purchase_price))*100)
                return (
                  <tr key={a.asset_id} style={{borderBottom:'1px solid #f1f5f9'}}>
                    <td style={{padding:'10px 8px',color:'#0f172a',fontWeight:600,fontSize:13}}>{a.asset_name}</td>
                    <td style={{padding:'10px 8px',color:'#64748b',fontSize:11}}>{a.asset_code}</td>
                    <td style={{padding:'10px 8px',color:'#475569',fontSize:12}}>{a.category}</td>
                    <td style={{padding:'10px 8px',color:'#475569',fontSize:12}}>{a.department}</td>
                    <td style={{padding:'10px 8px',color:'#475569',fontSize:12}}>{a.assigned_to||'Unassigned'}</td>
                    ...
                    <td style={{padding:'10px 8px'}}><button onClick={()=>{setEditAsset(a);setForm(a);setShowForm(true)}} style={{background:'#e2e8f0',border:'none',borderRadius:6,color:'#475569',padding:'4px 8px',cursor:'pointer',fontSize:12}}><Edit size={12}/></button></td>
                  </tr>
                )
              })}
            </tbody>
      ...
      {showForm && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={()=>setShowForm(false)}>
          <div style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:16,padding:24,width:580,maxHeight:'90vh',overflowY:'auto'}} onClick={e=>e.stopPropagation()}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}><h2 style={{color:'#0f172a',margin:0}}>{editAsset?'Edit Asset':'Add Asset'}</h2><button onClick={()=>setShowForm(false)} style={{background:'none',border:'none',color:'#475569',cursor:'pointer'}}><X size={20}/></button></div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
              ...
              <div><label style={{fontSize:11,color:'#64748b',display:'block',marginBottom:4}}>Status</label><select value={form.status} onChange={e=>setForm({...form,status:e.target.value})} style={{width:'100%',background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:8,color:'#0f172a',padding:'8px 12px',fontSize:13}}>{['Active','Under Maintenance','Disposed','Lost'].map(s=><option key={s}>{s}</option>)}</select></div>
            </div>
