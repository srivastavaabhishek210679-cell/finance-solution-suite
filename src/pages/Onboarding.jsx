    ...
    <div style={{minHeight:'100vh',background:'#f8fafc',fontFamily:'Inter,sans-serif',color:'#f8fafc'}}>
        ...
        <div style={{display:'flex',gap:8}}>
          {[1,2,3].map(s=>(
            <div key={s} style={{display:'flex',alignItems:'center',gap:6}}>
              <div style={{width:28,height:28,borderRadius:'50%',background:step>=s?'#10b981':'#e2e8f0',display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,fontWeight:700,color:step>=s?'#fff':'#64748b'}}>{step>s?'✓':s}</div>
              <span style={{fontSize:12,color:step>=s?'#10b981':'#64748b',display:s<3?'block':'block'}}>{['Select Modules','Choose Reports','Confirm'][s-1]}</span>
              {s<3 && <div style={{width:20,height:1,background:'#e2e8f0'}}></div>}
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{padding:24,maxWidth:1100,margin:'0 auto'}}>
        ...
        {step===1 && (
          <div>
            <div style={{marginBottom:24}}>
              <h2 style={{color:'#f8fafc',fontSize:22,fontWeight:700,margin:'0 0 8px'}}>Select Your Work Modules</h2>
            ...
            <div style={{display:'flex',gap:12,marginBottom:20,alignItems:'center'}}>
              <div style={{display:'flex',alignItems:'center',gap:8,background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:8,padding:'8px 14px',flex:1}}>
                <Search size={16} style={{color:'#64748b'}}/>
                <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search modules..." style={{background:'none',border:'none',color:'#f8fafc',fontSize:14,outline:'none',flex:1}}/>
              </div>
              <div style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:8,padding:'8px 14px',fontSize:13,color:'#64748b'}}>
            ...
            <div style={{display:'grid',gap:16}}>
              {filteredGroups.map(([domain, data])=>(
                <div key={domain} style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:12,overflow:'hidden'}}>
                  <div style={{padding:'12px 16px',background:data.color+'15',borderBottom:'1px solid #e2e8f0',display:'flex',alignItems:'center',gap:10}}>
                  ...
                  <div style={{padding:12,display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8}}>
                    {data.modules.map(mod=>{
                      ...
                      return (
                        <button key={mod.module_path} onClick={()=>toggleModule(mod)} style={{background:isSelected?data.color+'20':'#f8fafc',border:`1px solid ${isSelected?data.color:'#e2e8f0'}`,borderRadius:8,padding:'10px 14px',cursor:'pointer',display:'flex',alignItems:'center',gap:8,transition:'all 0.2s'}}>
                          <div style={{width:20,height:20,borderRadius:4,border:`2px solid ${isSelected?data.color:'#e2e8f0'}`,background:isSelected?data.color:'transparent',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                            {isSelected && <span style={{color:'#fff',fontSize:12}}>✓</span>}
                          </div>
                          <span style={{color:isSelected?'#f8fafc':'#475569',fontSize:13,textAlign:'left'}}>{mod.module_name}</span>
                        </button>
                      )
                    })}
                  </div>
        ...
        {step===2 && (
          <div>
            <div style={{marginBottom:24}}>
              <h2 style={{color:'#f8fafc',fontSize:22,fontWeight:700,margin:'0 0 8px'}}>Select Reports to Work With</h2>
            ...
            <div style={{display:'flex',gap:12,marginBottom:20,alignItems:'center'}}>
              <div style={{display:'flex',alignItems:'center',gap:8,background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:8,padding:'8px 14px',flex:1}}>
                <Search size={16} style={{color:'#64748b'}}/>
                <input value={reportSearch} onChange={e=>setReportSearch(e.target.value)} placeholder="Search reports..." style={{background:'none',border:'none',color:'#f8fafc',fontSize:14,outline:'none',flex:1}}/>
              </div>
              ...
              <button onClick={()=>setSelectedReports([])} style={{background:'#e2e8f0',border:'none',borderRadius:8,color:'#475569',padding:'8px 16px',cursor:'pointer',fontSize:13}}>Clear</button>
              <div style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:8,padding:'8px 14px',fontSize:13,color:'#10b981',fontWeight:600}}>
                ...
                return (
                  <div key={domain} style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:12,overflow:'hidden'}}>
                    <div style={{padding:'12px 16px',display:'flex',alignItems:'center',gap:12,cursor:'pointer'}} onClick={()=>toggleDomainExpand(domain_id)}>
                      <button onClick={e=>{e.stopPropagation();toggleAllReportsInDomain(domain_id)}} style={{width:20,height:20,borderRadius:4,border:`2px solid ${allSelected?'#10b981':someSelected?'#f59e0b':'#e2e8f0'}`,background:allSelected?'#10b981':someSelected?'#f59e0b20':'transparent',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',flexShrink:0,padding:0}}>
                        {allSelected && <span style={{color:'#fff',fontSize:10}}>✓</span>}
                        {someSelected && !allSelected && <span style={{color:'#f59e0b',fontSize:10}}>-</span>}
                      </button>
                      <span style={{color:'#f8fafc',fontWeight:600,fontSize:14,flex:1}}>{domain}</span>
                    ...
                    {isExpanded && (
                      <div style={{borderTop:'1px solid #e2e8f0',padding:12,display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:6,maxHeight:300,overflowY:'auto'}}>
                        {domReports.map(report=>{
                          const isSelected = !!selectedReports.find(r=>r.report_id===report.report_id)
                          return (
                            <button key={report.report_id} onClick={()=>toggleReport(report)} style={{background:isSelected?'#10b98115':'#f8fafc',border:`1px solid ${isSelected?'#10b981':'#e2e8f0'}`,borderRadius:6,padding:'8px 10px',cursor:'pointer',display:'flex',alignItems:'center',gap:8,textAlign:'left',transition:'all 0.2s'}}>
                              <div style={{width:16,height:16,borderRadius:3,border:`2px solid ${isSelected?'#10b981':'#e2e8f0'}`,background:isSelected?'#10b981':'transparent',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                                {isSelected && <span style={{color:'#fff',fontSize:9}}>✓</span>}
                              </div>
                              <div style={{flex:1,minWidth:0}}>
                                <div style={{color:isSelected?'#10b981':'#f8fafc',fontSize:12,fontWeight:500,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{report.name}</div>
            ...
            <div style={{display:'flex',justifyContent:'space-between',marginTop:24}}>
              <button onClick={()=>setStep(1)} style={{background:'#e2e8f0',border:'none',borderRadius:10,color:'#475569',padding:'12px 24px',fontSize:14,cursor:'pointer'}}>← Back</button>
        ...
        {step===3 && (
          <div>
            <div style={{marginBottom:24}}>
              <h2 style={{color:'#f8fafc',fontSize:22,fontWeight:700,margin:'0 0 8px'}}>Confirm Your Workspace</h2>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:20,marginBottom:24}}>
              <div style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:12,padding:20}}>
                <h3 style={{color:'#f8fafc',margin:'0 0 16px',fontSize:15,display:'flex',justifyContent:'space-between'}}>
                  <span>Selected Modules</span>
                  <span style={{color:'#10b981',fontSize:13}}>{selectedModules.length}</span>
                </h3>
                <div style={{display:'grid',gap:6}}>
                  {selectedModules.map(m=>(
                    <div key={m.module_path} style={{background:'#f8fafc',borderRadius:6,padding:'6px 10px',fontSize:13,color:'#475569',display:'flex',justifyContent:'space-between'}}>
              ...
              <div style={{background:'#ffffff',border:'1px solid #e2e8f0',borderRadius:12,padding:20}}>
                ...
                <div style={{maxHeight:300,overflowY:'auto',display:'grid',gap:4}}>
                  {selectedReports.map(r=>(
                    <div key={r.report_id} style={{background:'#f8fafc',borderRadius:6,padding:'6px 10px',fontSize:12,color:'#475569',display:'flex',justifyContent:'space-between'}}>
            ...
            <div style={{background:'#10b98115',border:'1px solid #10b98140',borderRadius:12,padding:20,marginBottom:24}}>
              <div style={{display:'flex',gap:12,alignItems:'center'}}>
                ...
                <div>
                  ...
                  <div style={{color:'#475569',fontSize:13}}>{selectedModules.length} modules • {selectedDomains.length} domains • {selectedReports.length} reports selected</div>
                </div>
              </div>
            </div>

            <div style={{display:'flex',justifyContent:'space-between'}}>
              <button onClick={()=>setStep(2)} style={{background:'#e2e8f0',border:'none',borderRadius:10,color:'#475569',padding:'12px 24px',fontSize:14,cursor:'pointer'}}>← Back</button>
